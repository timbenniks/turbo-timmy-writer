...
the create call. The transport requires a configured Person URN, Bearer token
with `w_member_social`, and explicit API version. A saved, Ready, current
LinkedIn-post variant must pass explicit immediate-publication confirmation;
the exact shared request object is persisted in `deliveries` before the call.
Generation and variant persistence never invoke it.
...
Publication-ready website metadata lives with the editable website variant,
not the canonical article: ISO publication date, external hero image URL, and
canonical tags are stored in its versioned metadata JSON. These fields are
optional at rest so pre-Phase-6 variants remain readable, but the publication
contract requires every field before rendering either exact repository preview
or attempting a future write. The two previews are generated by the same
validated serializer that will feed the GitHub adapter, preventing a separate
UI-only approximation from drifting from committed output.

The GitHub Contents adapter is server-only and receives its token, repository
allowlist, request implementation, and timeout through explicit construction.
It reads the configured branch before a caller chooses create or update, and an
update carries the previously observed blob SHA as an optimistic concurrency
...
Tiptap JSON is canonical. Plain text and Markdown are projections. On a document save, the server validates the document shape, derives plain text using the versioned serializer, and persists both atomically. Markdown may be generated on demand until a version or publication needs a stable snapshot. The Phase 7 history view reads owner-scoped immutable snapshots and compares their plain-text projections deterministically; current content participates as an unsnapshotted comparison input. Safe AI provenance may be joined by run ID, but prompts and generated outputs never enter the history response. Restore is an explicit, optimistic-concurrency mutation: one atomic statement checkpoints the current canonical state, replaces it from the selected owned snapshot, and records the restored result. Lifecycle status and derived variants are not silently changed.
...
Phase 7 adds canonical external hero metadata without requiring a provider.
The client previews the user-entered URL, while the server validates and stores
URL, required alt text, optional caption, and optional credit in canonical
article metadata under optimistic concurrency. Ordinary prose saves preserve
that metadata. Website variant generation copies the canonical URL
deterministically after AI output validation. An optional server-only
Cloudinary adapter accepts a bounded image, performs a signed upload with a
short timeout, and returns the same provider-neutral HTTPS shape. Its API
secret never enters the client bundle; the uploaded URL is not canonical until
Tim supplies alt text and explicitly saves it.
...
Application errors include a request correlation ID where practical. AI runs retain skill version, model, timing, token counts, status, and parse errors without secrets. Publication records retain provider identifiers, paths, commit SHA, status, and safe error summaries. Phase 7's private Insights view derives personal usage totals from these owner-scoped operational rows and canonical article metadata; it does not add tracking events or a third-party analytics script.
...
OpenAI, publisher, and optional Cloudinary variables are documented but are not
required for the core editor. CI builds use safe placeholders only when
build-time validation requires them; runtime secrets are never committed.
