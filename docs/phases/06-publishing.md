...
6. Validate a safe end-to-end publish/update against the configured repository. Complete on 2026-09-06: the real adapter created, inspected, and updated `src/content/writing/phase6-publisher-validation.md` on a disposable `phase6-validation-*` branch in `timbenniks/timbenniks-2026`; both commits returned valid distinct commit/blob SHAs, and the branch was deleted afterward. The live test now builds its fixture through the production serializer so future validation exercises schema-valid Astro frontmatter. No article reached either production branch and the app never invoked Vercel directly.
