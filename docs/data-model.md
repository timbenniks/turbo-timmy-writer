...
- Phase 7 quick captures remain canonical articles rather than introducing a second notes format. They use lifecycle status `idea` plus `metadata.entryKind` (`idea`, `fragment`, or `research-note`), supported Tiptap paragraph JSON, and its derived plain-text projection. Normal saves preserve the metadata kind.
- The repository writing importer maps source Markdown into the supported canonical editor vocabulary and derives plain text. It preserves source-marked drafts as `drafting`, treats the other article files as `published`, keeps slugs from frontmatter or the filename, copies HTTP(S) hero images into `metadata.heroImage` with the title as alt text, and records the exact original Markdown body in an immutable version with reason `import`. The section `index.md` is navigation rather than an article and is not imported.
- Reconcile is the default write path: dry-run first, then `--write`. It matches by slug or unique title, inserts new source files, remaps changed slugs, updates tags and hero images, snapshots body or title changes, and leaves unmatched local articles in place. `--replace` remains a destructive owner-scoped wipe and requires the direct database URL. These editable article rows do not replace the separate archive-document/chunk model.
